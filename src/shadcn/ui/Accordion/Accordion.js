...
const AccordionItem = React.forwardRef(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn(
      "accordion-btn border-[#191A23] border-[1px] rounded-[45px] py-6 px-[60px] transition-all duration-300 bg-[#F3F3F3] data-[state=open]:bg-[#B9FF66] hover:bg-[#B9FF66]",
      className
    )}
    {...props}
  />
  ...
  ({ className, children, ...props }, ref) => (
    <AccordionPrimitive.Header className="flex">
      <AccordionPrimitive.Trigger
        ref={ref}
        className={cn(
          "flex flex-1 items-center py-6  text-lg font-medium transition-all text-left ",
          className
        )}
        {...props}
      >
